...
- context-refresh v2 legacy owner was retired in a separate low-risk PIR-15 slice; its
  advisory bundle behavior is now retained by a Project Intelligence adapter helper.
- plan-item impact-map legacy owner was retired in a separate low-risk PIR-15 slice; its
...
- repo-index API production consumers now call through a Project Intelligence adapter,
  reducing source-derived direct legacy production consumers from 7 to 5 while preserving
  repository index behavior and leaving legacy deletion for the later consumer-zero gate.
...
- code-explorer legacy owner was retired in a separate low-risk PIR-15 slice; best-effort
...
Status: in_progress
Changed modules/files:
- agent/atlas_code_intel_service.py — removed the retired code-intelligence legacy owner after
  source-derived consumer-zero and rollback gates remained passed.
- agent/project_intelligence/adapters/code_intel.py — now owns retained read-only symbol,
  dependency, and related-test behavior behind ProjectIntelligenceCodeIntelAdapter.
- agent/project_intelligence/inspection/consumer_inventory.py, generated inventory/allowlist,
  consumer registry, and cutover artifacts — removed the retired legacy module from the legacy
  capability list while keeping source-derived direct legacy consumers at 0.
- tests and docs — updated code-intel imports and baseline locks for the retained adapter owner.
Evidence:
- python tools\generate_project_intelligence_consumer_inventory.py --root . --output
  docs\generated\atlas_project_intelligence_consumer_inventory.json -> production_entrypoints=32,
  legacy_consumers=0, facades=6, adapters=15, critical_findings=6.
- python - <<script invoking write_allowlist(...), write_lint_report(...),
  write_consumer_registry(...), write_consumer_cutover_gate(...)>> -> adapter_count=15,
  legacy_modules=6, legacy_production_consumers=0, allowed_dependency_count=0,
  lint_passed=true, registry_legacy_sum=0, cutover_passed=true.
- python -m py_compile agent\project_intelligence\adapters\code_intel.py
  app\api\atlas_code_intel.py agent\atlas_context_local_collectors.py
  agent\project_intelligence\inspection\consumer_inventory.py
  tests\test_atlas_code_intel_service.py tests\test_atlas_code_intel_api.py
  tests\test_project_intelligence_recovery_baseline.py
  tests\test_project_intelligence_baseline.py tests\test_project_twin_baseline.py -> compile OK.
- python -m pytest -q tests\test_atlas_code_intel_api.py
  tests\test_atlas_code_intel_service.py tests\test_project_intelligence_recovery_baseline.py
  tests\test_project_intelligence_pir14_legacy_dependency_lint.py
  tests\test_project_intelligence_pir15_repo_context_adapter.py
  tests\test_project_intelligence_pir15_inspection_adapter.py
  tests\test_project_intelligence_baseline.py tests\test_project_twin_baseline.py ->
  79 passed, 1 xfailed in 40.63s.
- python tools\run_pir15_retirement_gate.py --benchmark-report
  ca_data\atlas\pir15_live_benchmark_report.r12.json --consumer-registry
  ca_data\atlas\pir14_consumer_registry.current.json --rollout-evidence
  ca_data\atlas\pir14_rollout_evidence.current.json --consumer-cutover-gate
  ...
  status=passed, active_rollout=true, legacy_consumer_count=0,
  data_migration_evidence=passed, blocked_reasons=[].
- git diff | git apply --check -R -> exit 0.
Proof level: component_complete. This slice only removes one proven-zero legacy owner; PIR-15
continues for remaining retirement candidates and final package closure.

Work package: PIR-15 — Code explorer legacy owner removal
Status: in_progress
Changed modules/files:
- agent/atlas_code_explorer.py — removed the retired code-explorer legacy owner after
  source-derived consumer-zero, migration evidence, and rollback gates remained passed.
- agent/project_intelligence/adapters/code_explorer.py — now owns the retained best-effort
  read-only code excerpts, symbol extraction, related-test lookup, and research evidence behavior.
- agent/project_intelligence/inspection/consumer_inventory.py, generated inventory/allowlist,
  consumer registry, and cutover artifacts — removed the retired legacy module from the legacy
  capability list while keeping source-derived direct legacy consumers at 0.
- tests and docs — updated code-explorer imports and baseline locks for the retained adapter owner.
Evidence:
- python tools\generate_project_intelligence_consumer_inventory.py --root . --output
  docs\generated\atlas_project_intelligence_consumer_inventory.json -> production_entrypoints=32,
  legacy_consumers=0, facades=6, adapters=15, critical_findings=6.
- python - <<script invoking write_allowlist(...), write_lint_report(...),
  write_consumer_registry(...), write_consumer_cutover_gate(...)>> -> adapter_count=15,
  legacy_modules=7, legacy_production_consumers=0, allowed_dependency_count=0,
  lint_passed=true, registry_legacy_sum=0, cutover_passed=true.
- python -m py_compile agent\project_intelligence\adapters\code_explorer.py
  ...
  tests\test_atlas_code_explorer.py tests\test_project_intelligence_recovery_baseline.py
  tests\test_project_intelligence_baseline.py tests\test_project_twin_baseline.py -> compile OK.
...
Status: in_progress
Changed modules/files:
- agent/project_intelligence/adapters/code_intel.py — added a retained Project Intelligence
  adapter for read-only symbol, dependency, and related-test behavior.
- app/api/atlas_code_intel.py and agent/atlas_context_local_collectors.py — moved
  AtlasCodeIntelService construction behind ProjectIntelligenceCodeIntelAdapter.
- agent/project_intelligence/inspection/consumer_inventory.py, generated inventory/allowlist,
  consumer registry, and cutover artifacts — recorded the code-intel adapter and reduced direct
  legacy production consumers to 3.
- tests — updated baseline and PIR-15 regression locks; PIR0-C03 now passes normally because
  real Atlas API entrypoints no longer import legacy capability modules directly.
Evidence:
- python -m py_compile agent\project_intelligence\adapters\code_intel.py
  app\api\atlas_code_intel.py agent\atlas_context_local_collectors.py
  agent\project_intelligence\inspection\consumer_inventory.py
...
- python tools\generate_project_intelligence_consumer_inventory.py --root . --output
  docs\generated\atlas_project_intelligence_consumer_inventory.json -> production_entrypoints=32,
  legacy_consumers=3, facades=6, adapters=14, critical_findings=6.
- python - <<script invoking write_allowlist(...), write_lint_report(...),
  write_consumer_registry(...), write_consumer_cutover_gate(...)>> -> adapter_count=14,
  legacy_modules=8, legacy_production_consumers=3, allowed_dependency_count=3,
  lint_passed=true, registry_legacy_sum=0, cutover_passed=true.
- python tools\run_pir15_retirement_gate.py --benchmark-report
  ca_data\atlas\pir15_live_benchmark_report.r12.json --consumer-registry
  ca_data\atlas\pir14_consumer_registry.current.json --rollout-evidence
  ...
  data_migration_evidence=passed, blocked_reasons=[].
- python -m pytest -q tests\test_atlas_code_intel_api.py
  tests\test_atlas_code_intel_service.py tests\test_atlas_context_refresh_service.py
  tests\test_atlas_context_refresh_repo_context_root_persistence.py
  tests\test_project_intelligence_recovery_baseline.py
  tests\test_project_intelligence_pir14_legacy_dependency_lint.py
  tests\test_project_intelligence_pir15_repo_context_adapter.py
  tests\test_project_intelligence_pir15_inspection_adapter.py
  tests\test_project_intelligence_baseline.py tests\test_project_twin_baseline.py ->
  95 passed, 1 xfailed in 41.82s.
Proof level: production_connected. Real Atlas API and context-refresh local collector paths now
use the Project Intelligence adapter for code-intelligence behavior; this does not delete the
legacy code-intel owner.

Work package: PIR-15 — Repo index API adapter cutover
Status: in_progress
Changed modules/files:
- agent/project_intelligence/adapters/repo_index.py — added a retained Project Intelligence
  adapter for repository-index API policies, build, impact, related-test, latest, and result
  lookup behavior.
- app/api/atlas_repo_index.py and app/api/atlas_code_intel.py — moved repo-index service and
  storage construction behind ProjectIntelligenceRepoIndexAdapter without changing public routes.
- agent/project_intelligence/inspection/consumer_inventory.py, generated inventory/allowlist,
  consumer registry, and cutover artifacts — recorded the repo-index adapter and reduced direct
  legacy production consumers to 5.
- tests — added/updated regression locks proving repo-index API routes no longer import the
  legacy repository-index capability directly.
Evidence:
- python -m py_compile agent\project_intelligence\adapters\repo_index.py
  app\api\atlas_repo_index.py app\api\atlas_code_intel.py
  agent\project_intelligence\inspection\consumer_inventory.py
  tests\test_project_intelligence_recovery_baseline.py
  tests\test_project_intelligence_pir14_legacy_dependency_lint.py -> compile OK.
- python tools\generate_project_intelligence_consumer_inventory.py --root . --output
  docs\generated\atlas_project_intelligence_consumer_inventory.json -> production_entrypoints=32,
  legacy_consumers=5, facades=6, adapters=13, critical_findings=6.
- python - <<script invoking write_allowlist(...), write_lint_report(...),
  write_consumer_registry(...), write_consumer_cutover_gate(...)>> -> adapter_count=13,
  legacy_modules=8, legacy_production_consumers=5, allowed_dependency_count=5,
  lint_passed=true, registry_legacy_sum=0, cutover_passed=true.
- python tools\run_pir15_retirement_gate.py --benchmark-report
  ca_data\atlas\pir15_live_benchmark_report.r12.json --consumer-registry
  ca_data\atlas\pir14_consumer_registry.current.json --rollout-evidence
  ...
  status=passed, active_rollout=true, legacy_consumer_count=0,
  data_migration_evidence=passed, blocked_reasons=[].
- python -m pytest -q tests\test_atlas_repo_index_api.py
  tests\test_atlas_repo_index_service.py tests\test_atlas_code_intel_api.py
  tests\test_project_intelligence_recovery_baseline.py
  tests\test_project_intelligence_pir14_legacy_dependency_lint.py
  tests\test_project_intelligence_pir15_repo_context_adapter.py
  tests\test_project_intelligence_pir15_inspection_adapter.py -> 31 passed, 2 xfailed
  in 39.96s.
Proof level: production_connected. Real Atlas API routes are now connected through the
Project Intelligence adapter for repo-index behavior; this does not delete the legacy
repo-index owner.
...
Status: in_progress
Changed modules/files:
- agent/atlas_context_refresh_service.py — removed the retired v1 context-refresh legacy owner
  after consumer-zero, data migration, and rollback gates remained passed.
- agent/project_intelligence/adapters/context_refresh_v1.py — added the retained Project
  Intelligence adapter that preserves advisory bundle creation, event recording, and result-path
  persistence behavior.
- agent/project_intelligence/adapters/atlas_context_refresh.py — now constructs
  ProjectIntelligenceContextRefreshAdapter for v1 refresh requests.
- agent/project_intelligence/inspection/consumer_inventory.py, generated inventory/allowlist,
  migration docs, current status, and focused tests — updated owner inventory and retirement
  evidence.
Executed commands and exact results:
- python -m py_compile agent\project_intelligence\adapters\context_refresh_v1.py
  agent\project_intelligence\adapters\atlas_context_refresh.py
  agent\project_intelligence\inspection\consumer_inventory.py
  tests\test_atlas_context_refresh_service.py
  tests\test_atlas_context_refresh_repo_context_root_persistence.py -> compile OK.
- python -m pytest -q tests\test_atlas_context_refresh_service.py
  tests\test_atlas_context_refresh_repo_context_root_persistence.py
  tests\test_atlas_context_refresh_v2_api.py
  tests\test_atlas_context_refresh_v2_service.py
  tests\test_project_intelligence_pir15_legacy_internal_inventory.py
  tests\test_project_intelligence_pir15_inspection_adapter.py
  tests\test_project_intelligence_pir15_repo_context_adapter.py
  tests\test_project_intelligence_recovery_baseline.py
  ...
  tests\test_project_intelligence_pir14_consumer_registry.py
  tests\test_project_intelligence_pir15_data_migration_evidence.py
  tests\test_project_intelligence_pir15_retirement_gate.py -> 87 passed, 2 xfailed in 52.98s.
- python tools\generate_project_intelligence_consumer_inventory.py --root . --output
  ...
  -> adapter_count=11, legacy_modules=9, legacy_production_consumers=7,
...
- docs/generated/atlas_project_intelligence_consumer_inventory.json no longer lists
  agent.atlas_context_refresh_service as a legacy owner and records
  agent.project_intelligence.adapters.context_refresh_v1 as a present adapter.
...
Safety invariants checked: context-refresh v1 remains advisory-only, does not execute shell/git,
  preserves policy limits and web/deep-research gating, and does not move Requirement, PlanPool,
  Proposal/Safe Apply, Verification, or rollout authority.
...
```text
Work package: PIR-15 — Plan-item impact-map legacy owner removal
Status: in_progress
Changed modules/files:
- agent/atlas_plan_item_impact_map_service.py — removed the retired impact-map legacy owner after
  consumer-zero, data migration, and rollback gates remained passed.
- agent/project_intelligence/adapters/plan_item_impact_map.py — added the retained Project
  Intelligence helper that preserves advisory plan-item impact-map behavior.
- agent/project_intelligence/adapters/context_refresh_v2.py,
  agent/project_intelligence/adapters/atlas_repo_context.py, and
  agent/project_intelligence/adapters/planner_packaging_v2.py — now call
  ProjectIntelligencePlanItemImpactMapAdapter instead of the retired legacy owner.
- agent/project_intelligence/inspection/consumer_inventory.py, generated inventory/allowlist,
  migration docs, current status, and focused tests — updated owner inventory and retirement
  evidence.
Executed commands and exact results:
- python -m py_compile agent\project_intelligence\adapters\plan_item_impact_map.py
  agent\project_intelligence\adapters\context_refresh_v2.py
  agent\project_intelligence\adapters\atlas_repo_context.py
  agent\project_intelligence\adapters\planner_packaging_v2.py
  agent\project_intelligence\inspection\consumer_inventory.py
  tests\test_atlas_plan_item_impact_map_service.py
  tests\test_atlas_plan_item_impact_map_api.py
  tests\test_atlas_plan_item_impact_map_planpool_integration.py -> compile OK.
- python -m pytest -q tests\test_atlas_plan_item_impact_map_service.py
  tests\test_atlas_plan_item_impact_map_api.py
  tests\test_atlas_plan_item_impact_map_planpool_integration.py
  tests\test_atlas_plan_item_impact_map_safety_contract.py
  tests\test_atlas_context_refresh_v2_service.py
  ...
  tests\test_project_intelligence_pir14_consumer_registry.py
  tests\test_project_intelligence_pir15_data_migration_evidence.py
  tests\test_project_intelligence_pir15_retirement_gate.py -> 88 passed, 2 xfailed in 51.22s.
- python tools\generate_project_intelligence_consumer_inventory.py --root . --output
  docs\generated\atlas_project_intelligence_consumer_inventory.json; python <<regenerate
  current allowlist, lint, registry, and cutover artifacts>>
  -> adapter_count=10, legacy_modules=10, legacy_production_consumers=7,
  allowed_dependency_count=7, lint_passed=true, registry_legacy_sum=0, cutover_passed=true.
- python tools\run_pir15_retirement_gate.py --benchmark-report
  ca_data\atlas\pir15_live_benchmark_report.r12.json --consumer-registry
  ca_data\atlas\pir14_consumer_registry.current.json --rollout-evidence
...
Evidence details:
- docs/generated/atlas_project_intelligence_consumer_inventory.json no longer lists
  agent.atlas_plan_item_impact_map_service as a legacy owner and records
  agent.project_intelligence.adapters.plan_item_impact_map as a present adapter.
- PIR-15 retirement gate remains passed after deletion with legacy_consumer_count=0,
  data_migration_verified=true, and blocked_reasons=[].
Unavailable checks: removal of remaining proven-zero legacy owners remains unclaimed.
Safety invariants checked: impact-map packaging remains advisory-only, no-execution,
...
```text
Work package: PIR-15 — Context refresh v2 legacy owner removal
Status: in_progress
Changed modules/files:
- agent/atlas_context_refresh_v2_service.py — removed the retired v2 legacy owner after
  consumer-zero, data migration, and rollback gates remained passed.
- agent/project_intelligence/adapters/context_refresh_v2.py — added the retained Project
  Intelligence helper that preserves context-refresh v2 advisory bundle behavior.
- agent/project_intelligence/adapters/atlas_context_refresh.py and
  agent/project_intelligence/adapters/planner_packaging_v2.py — now call
  ProjectIntelligenceContextRefreshV2Adapter instead of the retired legacy owner.
- agent/project_intelligence/inspection/consumer_inventory.py, generated inventory/allowlist,
  migration docs, current status, and focused tests — updated owner inventory and retirement
  evidence.
Executed commands and exact results:
- python -m py_compile agent\project_intelligence\adapters\context_refresh_v2.py
  agent\project_intelligence\adapters\atlas_context_refresh.py
  agent\project_intelligence\adapters\planner_packaging_v2.py
  agent\project_intelligence\inspection\consumer_inventory.py
  tests\test_atlas_context_refresh_v2_service.py tests\test_atlas_context_refresh_v2_api.py
  tests\test_atlas_context_refresh_v2_safety_contract.py -> compile OK.
- python -m pytest -q tests\test_atlas_context_refresh_v2_service.py
  tests\test_atlas_context_refresh_v2_api.py
  tests\test_atlas_context_refresh_v2_safety_contract.py
  tests\test_project_intelligence_pir15_planner_packaging_retirement.py
  tests\test_project_intelligence_pir15_legacy_internal_inventory.py
  tests\test_project_intelligence_pir15_inspection_adapter.py
  tests\test_project_intelligence_pir15_repo_context_adapter.py
  ...
  tests\test_project_intelligence_pir14_consumer_registry.py
  tests\test_project_intelligence_pir15_data_migration_evidence.py
  tests\test_project_intelligence_pir15_retirement_gate.py -> 84 passed, 2 xfailed in 52.67s.
- python tools\generate_project_intelligence_consumer_inventory.py --root . --output
  docs\generated\atlas_project_intelligence_consumer_inventory.json; python <<regenerate
  current allowlist, lint, registry, and cutover artifacts>>
  -> adapter_count=9, legacy_modules=11, legacy_production_consumers=7,
  allowed_dependency_count=7, lint_passed=true, registry_legacy_sum=0, cutover_passed=true.
- python tools\run_pir15_retirement_gate.py --benchmark-report
  ca_data\atlas\pir15_live_benchmark_report.r12.json --consumer-registry
  ca_data\atlas\pir14_consumer_registry.current.json --rollout-evidence
...
Evidence details:
- docs/generated/atlas_project_intelligence_consumer_inventory.json no longer lists
  agent.atlas_context_refresh_v2_service as a legacy owner and records
  agent.project_intelligence.adapters.context_refresh_v2 as a present adapter.
- PIR-15 retirement gate remains passed after deletion with legacy_consumer_count=0,
  data_migration_verified=true, and blocked_reasons=[].
Unavailable checks: removal of remaining proven-zero legacy owners remains unclaimed.
Safety invariants checked: context-refresh v2 remains advisory-only, no-execution,
  no-auto-build, and manual-command-suggestion-only; no Requirement, PlanPool, Proposal/Safe
  Apply, Verification, or rollout authority was moved.
Migration/rollout state: default rollout remains off; exactly one additional legacy source owner
  was deleted in this slice.
...
Status: in_progress
Changed modules/files:
- agent/atlas_supervised_handoff_retry_service.py — constructs its default bounded-retry
  context-refresh dependency through the Project Intelligence context-refresh adapter.
- agent/atlas_supervised_handoff_verification_service.py — constructs its default
  context-refresh dependency through the same adapter, using the service storage root and journal.
- docs/generated/atlas_project_intelligence_consumer_inventory.json and
  docs/generated/atlas_project_intelligence_legacy_dependency_allowlist.json — regenerated from
  the current checkout after agent-service cutover.
- tests/test_project_intelligence_pir15_legacy_internal_inventory.py and
  tests/test_project_intelligence_pir14_legacy_dependency_lint.py — updated coverage for
  legacy_context_refresh consumer-zero and reduced direct allowlist.
Executed commands and exact results:
- python -m py_compile agent\atlas_supervised_handoff_retry_service.py
  agent\atlas_supervised_handoff_verification_service.py
  agent\project_intelligence\adapters\atlas_context_refresh.py -> compile OK.
- python -m pytest -q tests\test_atlas_supervised_handoff_retry_service.py
  tests\test_atlas_supervised_handoff_verification_service.py
  tests\test_project_intelligence_pir15_legacy_internal_inventory.py
  tests\test_project_intelligence_recovery_baseline.py
  tests\test_project_intelligence_pir14_legacy_dependency_lint.py
  tests\test_project_intelligence_pir14_consumer_registry.py
  tests\test_project_intelligence_pir15_retirement_gate.py -> 28 passed, 2 xfailed in 37.25s.
- python <<regenerate current inventory, allowlist, rollout, lint, cutover, and registry artifacts>>
  -> inventory_legacy=8, allowed_dependency_count=8, lint_passed=true,
  registry_legacy_sum=1, post_apply_refresh.legacy_consumer_count=0,
...
- python tools\run_pir15_retirement_gate.py --benchmark-report
  ...
  ca_data\atlas\pir15_active_rollout_transition.current.json --output-json
  ca_data\atlas\pir15_retirement_gate.current.json --docs-updated --allow-blocked-exit-zero
  -> exit 0; status=blocked, active_rollout=true, legacy_consumer_count=1,
  blocked_reasons=[data_migration_not_verified, legacy_capability_retirement_not_ready].
Evidence details:
- Generated inventory now records agent.atlas_context_refresh_service and
  agent.atlas_context_refresh_v2_service with production_consumer_count=0 and
  adapter_consumer_count=1.
- Direct legacy dependency allowlist now records 8 observed production dependencies, down from 10
  after the context-refresh API adapter cutover.
- PIR-15 retirement gate now reports legacy_context_refresh=0 and legacy_verification_gate=1; the
  only remaining direct retirement-gate consumer is the verification gate authority in
  app/api/atlas_pipeline.py.
Unavailable checks: data migration verification, verification-gate consumer-zero/retained-role
  decision, repair/Greenfield shadow and rollback parity, actual legacy removal, rollback after
  each removal, and master Definition of Done remain unclaimed.
Safety invariants checked: supervised handoff retry and verification still receive a
  context-refresh dependency with no execution approval, Safe Apply, rollback, or rollout
  transition behavior added.
Migration/rollout state: default rollout remains off; no legacy source path was deleted.
Known limitations: PIR-15 acceptance is not complete because verification gate remains a direct
  production consumer, data migration is not verified, and repair/Greenfield shadow/rollback
  parity is still incomplete for consumer-zero capabilities.
Next package: PIR-15 — resolve the verification-gate retirement classification without weakening
  canonical verification authority, then re-run retirement/data-migration gates.
Blocker: none; remaining work is the next PIR-15 gate/retirement slice.
```
