...
| H5 | feat/forge-rescue-execution-wiring | **(救出配線)** 救出プランを ExecutionPolicySelector に配線（全構築軸が測定済み全滅時のみ発火、実行時に救出 method/patch mode を適用、`rescue_fallback_model` で昇格先を渡せる） | ☑ merged |

---

## 進捗ログ

- 2026-06-21: **H5 救出プランを実行経路に配線**（リポジトリ最新化後: origin/main の Twin Assist トラック #1989-2000 を取り込み、その新 `execution_policy.py` に再適用）。`ExecutionPolicySelector.select` が、全4構築軸（structured/patch/edit/anchor）が**測定済みかつ全滅**した場合のみ `CapabilityRescuePlanner` を発火し、実行ポリシーの `method_variant`/`method_fallbacks`/`patch_construction_mode` を救出プラン（deterministic_text / review_only / escalate_fallback_model）で上書きし `capability_rescue=<level>` を reasons に追記。部分測定・未測定は router 既定に委ね非回帰。`rescue_fallback_model` 引数で昇格先を渡せる。Safe Apply/RemotePublish gate と review_only 終端は常時維持。TA トラックの twin_assist 添付とは独立・両立。`tests/test_forge_execution_rescue_wiring.py` 5 passed + 広域回帰（router/exec/atlas/eval/TA twin assist 群）69 passed。
