...
## Launcher early-failure behavior

If ModelManager reports `auto_load_failed=true`, the launcher should stop waiting for LLM readiness instead of waiting the full timeout.

Expected warning:

```text
LLM auto-load failed early: runpod_gpu_validation_failed_without_oom
See /debug/model-startup
Run collect_runtime_snapshot.sh
```

This does not mean FastAPI failed. It means the web UI may open, but LLM-backed features will fail until CUDA / llama-server GPU offload is fixed.
