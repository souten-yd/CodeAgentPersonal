...
| `AtlasPlanItemImpactMapService` | `agent/atlas_context_refresh_v2_service.py:AtlasContextRefreshV2Service`; `agent/atlas_planner_packaging_v2_service.py:AtlasPlannerPackagingV2Service`; `app/api/atlas_pipeline.py`; `app/api/atlas_repo_context.py` | agent services + API |
| `AtlasRepoContextService` | `agent/atlas_context_refresh_service.py:AtlasContextRefreshService`; `agent/atlas_repo_context_planner_packager.py:AtlasRepoContextPlannerPackager`; `app/api/atlas_pipeline.py`; `app/api/atlas_repo_context.py` | agent services + API |
| `AtlasRepoContextPlannerPackager` | `agent/atlas_context_refresh_service.py`; `agent/atlas_planner_packaging_v2_service.py`; `agent/atlas_plan_item_impact_map_service.py`; `agent/atlas_verification_planning_service.py`; `app/api/atlas_pipeline.py`; `app/api/atlas_repo_context.py` | agent services + API |
...
| `AtlasContextRefreshV2Service` | `agent/atlas_planner_packaging_v2_service.py`; `app/api/atlas_context_refresh.py` | agent service + API |
| `AtlasPlannerPackagingV2Service` | `agent/atlas_verification_recommendation_service.py:AtlasVerificationRecommendationService`; `app/api/atlas_pipeline.py`; `app/api/atlas_repo_context.py` | agent service + API |
...
2. **`agent/atlas_context_refresh_service.py` is a hub**: it consumes RepoContext,
   PlannerPackager, and the local collectors, and is itself consumed by the
...
3. **Context construction has layered consumers** (ContextBuilder → loop; RepoContext →
   PlannerPackager → PlannerPackagingV2 → VerificationRecommendation). Cutover must be
