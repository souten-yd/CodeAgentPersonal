...
- PR-ATLAS-PIPE-28: Manual post-apply verification UX for draft safe_apply results / no auto debug
...
- PR-ATLAS-PIPE-27 does not execute DeepResearch/Web jobs and does not add Task/Agent APIs.

## Next Instruction

PR-ATLAS-PIPE-28を実装する。
draft由来PlanItemのmanual safe_apply結果から、Post-Apply Verification panelへ自然に誘導し、手動verification UX/E2Eを整える。ただし自動verification・自動debug loopはまだ行わない。
