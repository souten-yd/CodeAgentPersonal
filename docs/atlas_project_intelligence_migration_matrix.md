...
|---|---|---|---|---|---|---|
| 1 | Repository enumeration | `ProjectIntelligenceRepoIndexService` (legacy `AtlasRepoIndexService` retired in PIR-15) (+ `_iter_project_files`, twin `static_graph`) | ADAPT → REPLACE | DigitalTwinModule | PI-1 facade, PI-6 deep graph, PI-23 cutover | consumer-zero on Repo Index after PI-23 |
| 2 | Symbol extraction | `ProjectIntelligenceCodeIntelAdapter.build_symbol_index` (legacy `AtlasCodeIntelService` retired in PIR-15); `ProjectIntelligenceCodeExplorerAdapter.extract_symbols` (legacy `atlas_code_explorer.extract_symbols` retired in PIR-15); twin `static_graph` | ADAPT → REPLACE | DigitalTwin semantic analyzer | PI-6, PI-23 | parity vs CodeIntel pinned by baseline test |
| 3 | Dependency/import graph | `ProjectIntelligenceCodeIntelAdapter.build_dependency_graph` (legacy `AtlasCodeIntelService` retired in PIR-15); twin `static_graph` | ADAPT → REPLACE | DigitalTwin semantic analyzer | PI-6, PI-23 | parity on edge set + scope |
...
| 5 | Related-test discovery | `ProjectIntelligenceCodeExplorerAdapter.find_related_tests` (legacy `atlas_code_explorer.find_related_tests` retired in PIR-15); `ProjectIntelligenceCodeIntelAdapter` related-tests (legacy `AtlasCodeIntelService` retired in PIR-15); `atlas_test_impl_linker.find_implementation_item` | ADAPT → REPLACE | DigitalTwin impact/test selection | PI-9, PI-23 | parity vs all 3 legacy mechanisms |
| 6 | API route discovery | `ProjectIntelligenceRepoIndexService` (legacy `AtlasRepoIndexService` retired in PIR-15) + `atlas_repo_index_policies`; twin route projection | ADAPT → REPLACE | DigitalTwin API graph | PI-6 | route-set parity |
...
| 9 | Repository context | `ProjectIntelligenceRepoContextService` (legacy `AtlasRepoContextService` retired in PIR-15); `ContextBuilder`/`TaskV2ContextBuilder`; `ProjectIntelligenceRepoContextPackager` (legacy `AtlasRepoContextPlannerPackager` retired in PIR-15) | ADAPT → REPLACE | ProjectIntelligence context packages | PI-9, PI-16, PI-23, PIR-15 | shadow parity + consumer-zero + rollback proof |
...
| 11 | Context refresh | `ProjectIntelligenceContextRefreshAdapter` (legacy `AtlasContextRefreshService` retired in PIR-15); `ProjectIntelligenceContextRefreshV2Adapter` (legacy `AtlasContextRefreshV2Service` retired in PIR-15); `atlas_context_local_collectors` | ADAPT → REPLACE | ProjectIntelligence lifecycle/context | PI-9, PI-17, PI-23, PIR-15 | parity across handoff/retry/autopilot consumers + rollback proof |
| 12 | Impact map | `ProjectIntelligencePlanItemImpactMapAdapter` (legacy `AtlasPlanItemImpactMapService` retired in PIR-15); twin `analysis.GraphAnalysisService.assess_impact` | ADAPT → REPLACE | DigitalTwin impact query + Atlas adapter | PI-9, PI-23, PIR-15 | impact precision/recall parity + rollback proof |
