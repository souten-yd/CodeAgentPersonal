...
## PR4.56 audio diagnostic ownership note

PR4.56 moved `/audio/runtime/debug` from `app/api/runtime_controls.py` to `app/api/audio.py` with the same production `main.py` provider and a side-effect-free `create_app()` fallback. Runtime-controls still owns LLM/search/streaming controls, `/runtime/cuda-debug`, and `/debug/model-startup`.
