...
Updated after POST-SCALE-160 stable runtime mutation apply record.
...
- Stable runtime mutation apply is metadata-only: it records a strict approved mutation record from a ready gate, and does not switch release pointers, execute recovery, run commands, or perform git operations.
...
## Next narrow work
- POST-SCALE-160-DIRECT-MERGE-GATE: prepare direct merge policy only after stable runtime mutation apply is recorded and still leaves remote push, self-apply, pointer switching, arbitrary command execution, and Vue authority disabled.
