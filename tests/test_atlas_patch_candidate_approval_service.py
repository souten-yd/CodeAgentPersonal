...
def _seed(tmp_path):
    ...
    rr.joinpath('regen_1.json').write_text(json.dumps({'status':'proposal_ready','errors':[],'metadata':{'side_effects':{'safe_apply_executed':False,'verification_executed':False}},'candidate':{'proposal_id':'prop1','status':'proposal_ready','approval_required':True,'approval_status':'pending','safe_apply_ready':False,'patch':'diff --git a/a.py b/a.py\n--- a/a.py\n+++ b/a.py\n@@\n-old\n+new\n','patch_format':'unified_diff','target_files':['a.py'],'warnings':[],'errors':[]}}))
    return storage
...
def test_regen_errors_block_approval(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path); s=_seed(tmp_path)
    p = Path('ca_data/atlas/patch_regen/pool1/regen_1.json')
    d = json.loads(p.read_text()); d['errors'] = ['x']; p.write_text(json.dumps(d))
    res=AtlasPatchCandidateApprovalService(storage=s).decide(AtlasPatchCandidateApprovalRequest(pool_id='pool1', item_id='i1', regen_run_id='regen_1', decision='approve'))
    assert res.status == 'blocked' and 'regen_errors_present' in res.warnings


def test_side_effects_true_blocks_approval(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path); s=_seed(tmp_path)
    p = Path('ca_data/atlas/patch_regen/pool1/regen_1.json')
    d = json.loads(p.read_text()); d['metadata']['side_effects']['safe_apply_executed'] = True; p.write_text(json.dumps(d))
    res=AtlasPatchCandidateApprovalService(storage=s).decide(AtlasPatchCandidateApprovalRequest(pool_id='pool1', item_id='i1', regen_run_id='regen_1', decision='approve'))
    assert res.status == 'blocked' and 'side_effects_detected' in res.warnings
