...
def test_impact_map_build_failure_non_blocking(tmp_path: Path, monkeypatch):
    svc = AtlasContextRefreshV2Service(tmp_path)
...
def test_metadata_safety_flags(tmp_path: Path):
    r = AtlasContextRefreshV2Service(tmp_path).refresh(AtlasContextRefreshV2Request(project_path=str(tmp_path), plan_pool={'items': []}))
    m = r.metadata
