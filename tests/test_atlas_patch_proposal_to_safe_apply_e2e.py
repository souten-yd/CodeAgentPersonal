...
def _load_plan_pool_json(pool_id):
    p = Path(main.app.state.atlas_ca_data_dir) / 'atlas' / 'workspaces' / 'default' / 'plan_pools' / pool_id / 'plan_pool.json'
    import json
    return p, json.loads(p.read_text(encoding='utf-8'))

def _seed_patch_source(pool_id, item_id, patch_proposal):
    p, pool = _load_plan_pool_json(pool_id)
...
def test_patch_proposal_draft_safe_apply_updates_file_from_unified_diff(tmp_path):
    ...
    _, pool_json = _load_plan_pool_json(pid)
    assert pool_json['project_path'] == str(repo)
    _seed_patch_source(pid, iid, {'status':'approved','proposal_id':'pp1','risk_level':'low','target_files':['app.py'],'unified_diff_preview':'--- a/app.py\n+++ b/app.py\n@@\n-print("old")\n+print("new")\n'})
    _, pool_json = _load_plan_pool_json(pid)
    assert pool_json['project_path'] == str(repo)
    ...
    assert _load_plan_pool_json(pid)[1]['project_path'] == str(repo)
    ...
    assert r['metadata']['workspace_root'] == str(repo.resolve())
    assert r['metadata']['change_snapshot']['workspace_root'] == str(repo.resolve())
...
def test_patch_proposal_draft_blocks_when_no_executable_change_content(tmp_path):
    ...
    pool = _create_pool(c); pid = pool['pool_id']; iid = pool['plan_pool']['items'][0]['item_id']
    _set_project_path(tmp_path, pid, repo)
    _seed_patch_source(pid, iid, {'status':'approved','proposal_id':'pp1','risk_level':'low','target_files':['app.py']})
    _, draft_id = _create_and_approve_draft(c, pid, iid)
    r = c.post('/api/atlas/safe-apply/execute', json={'pool_id': pid, 'item_id': draft_id}).json()
...
def test_patch_proposal_draft_metadata_contains_executor_readable_patch(tmp_path):
    ...
    _seed_patch_source(pid, iid, {'status':'approved','proposal_id':'pp1','risk_level':'low','target_files':['app.py'],'unified_diff_preview': unified_diff_preview})
