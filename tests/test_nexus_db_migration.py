...
class NexusDbMigrationLegacyNullTests(unittest.TestCase):
    def test_migrates_empty_linked_document_id_to_null_without_fk_violation(self) -> None:
        ...
        conn.execute(
            ...
            VALUES('job-legacy', 'running', '2026-01-01T00:00:00+00:00', '2026-01-01T00:00:00+00:00')
            """
        )
        conn.execute(
            """
        ...
        migrated = conn.execute(
            "SELECT linked_document_id FROM nexus_sources WHERE source_id = 'src-legacy'"
        ).fetchone()
        self.assertIsNone(migrated["linked_document_id"])
