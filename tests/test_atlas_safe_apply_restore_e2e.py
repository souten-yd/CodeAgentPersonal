...
def _prepare_item(tmp_path, pool_id, item_id, action_type, target, content):
    _mutate_item(
        ...
        status='ready',
        target_files=[target],
        metadata={'action_type': action_type, 'approval': {'decision': 'approved'}, 'proposed_content': content},
    )
...
def test_safe_apply_snapshot_and_executor_use_same_workspace_root(tmp_path):
    ...
    assert m['executor_result']['actual_file_changed'] is True
    assert m['executor_result']['changed_files'] == ['a.txt']
    assert m['workspace_root'] == str(repo.resolve())
    assert m['change_snapshot']['workspace_root'] == str(repo.resolve())
    assert m['workspace_root'] == m['change_snapshot']['workspace_root']
    manifest = __import__('json').loads(Path(m['change_snapshot']['manifest_path']).read_text(encoding='utf-8'))
    assert manifest['target_files'][0]['path'] == 'a.txt'


def test_safe_apply_create_then_restore_skips_delete_without_confirmation(tmp_path):
    _clear_safe_apply_state(); c = _client(tmp_path)
    repo = tmp_path / 'repo'; repo.mkdir()
    main.app.state.atlas_implementation_executor = AtlasFileSafeApplyExecutor(workspace_root=repo)
    pool = _create_pool(c); pid = pool['pool_id']; iid = pool['plan_pool']['items'][0]['item_id']
    p = Path(tmp_path) / 'atlas' / 'workspaces' / 'default' / 'plan_pools' / pid / 'plan_pool.json'; import json; d = json.loads(p.read_text()); d['project_path'] = str(repo); p.write_text(json.dumps(d, ensure_ascii=False, indent=2))
    _prepare_item(tmp_path, pid, iid, 'create', 'new.txt', 'created\n')
    r = c.post('/api/atlas/safe-apply/execute', json={'pool_id': pid, 'item_id': iid}).json(); assert r['status'] == 'applied'
    assert (repo / 'new.txt').read_text() == 'created\n'
    manifest = r['metadata']['change_snapshot']['manifest_path']
    rr = c.post('/api/atlas/change-snapshots/restore', json={'pool_id': pid, 'item_id': iid, 'manifest_path': manifest, 'confirm_delete_missing_before': False}).json()
    file_results = rr.get('file_results') or []
    assert any(fr.get('skip_reason') == 'manual_confirm_required_for_delete' for fr in file_results)
    assert (repo / 'new.txt').exists()


def test_safe_apply_create_then_restore_deletes_with_confirmation(tmp_path):
    _clear_safe_apply_state(); c = _client(tmp_path)
    repo = tmp_path / 'repo'; repo.mkdir()
    main.app.state.atlas_implementation_executor = AtlasFileSafeApplyExecutor(workspace_root=repo)
    pool = _create_pool(c); pid = pool['pool_id']; iid = pool['plan_pool']['items'][0]['item_id']
    ...
    r = c.post('/api/atlas/safe-apply/execute', json={'pool_id': pid, 'item_id': iid}).json(); assert r['status'] == 'applied'
    manifest = r['metadata']['change_snapshot']['manifest_path']
    rr = c.post('/api/atlas/change-snapshots/restore', json={'pool_id': pid, 'item_id': iid, 'manifest_path': manifest, 'confirm_delete_missing_before': True}).json()
    assert rr.get('deleted_count') == 1
    assert any(fr.get('deleted') is True for fr in (rr.get('file_results') or []))
    assert not (repo / 'new.txt').exists()


def test_restore_after_safe_apply_does_not_auto_run_followups(tmp_path):
    _clear_safe_apply_state(); c = _client(tmp_path)
    repo = tmp_path / 'repo'; repo.mkdir(); (repo / 'sample.txt').write_text('before\n', encoding='utf-8')
    main.app.state.atlas_implementation_executor = AtlasFileSafeApplyExecutor(workspace_root=repo)
    pool = _create_pool(c); pid = pool['pool_id']; iid = pool['plan_pool']['items'][0]['item_id']
    p = Path(tmp_path) / 'atlas' / 'workspaces' / 'default' / 'plan_pools' / pid / 'plan_pool.json'; import json; d = json.loads(p.read_text()); d['project_path'] = str(repo); p.write_text(json.dumps(d, ensure_ascii=False, indent=2))
    _prepare_item(tmp_path, pid, iid, 'update', 'sample.txt', 'after\n')
    r = c.post('/api/atlas/safe-apply/execute', json={'pool_id': pid, 'item_id': iid}).json(); manifest = r['metadata']['change_snapshot']['manifest_path']
    rr = c.post('/api/atlas/change-snapshots/restore', json={'pool_id': pid, 'item_id': iid, 'manifest_path': manifest, 'confirm_delete_missing_before': False}).json()
    events = rr.get('events') or []
    types = [e.get('event_type') for e in events if isinstance(e, dict)]
    for forbidden in ('verification_manual_started', 'debug_review_manual_started', 'patch_proposal_manual_started', 'safe_apply_manual_started'):
