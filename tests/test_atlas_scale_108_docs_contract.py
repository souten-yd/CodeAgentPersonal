...
def test_roadmap_sections_are_strict_and_track_progression():
    ...
    assert "Planned UI track: return to PR-ATLAS-SCALE-110 automation track" in current
    assert "next work is PR-ATLAS-SCALE-110" in current
    ...
    assert "Current automation track: PR-ATLAS-SCALE-109" not in current
    assert "Next automation track: PR-ATLAS-SCALE-109" not in current
