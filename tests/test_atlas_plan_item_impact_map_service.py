...
def test_empty_plan_pool_non_blocking(tmp_path):
    out = ProjectIntelligencePlanItemImpactMapAdapter(tmp_path).build_map(_req(tmp_path, {}))
...
def test_one_entry_per_item(tmp_path):
    ...
    out = ProjectIntelligencePlanItemImpactMapAdapter(tmp_path).build_map(_req(tmp_path, pool))
