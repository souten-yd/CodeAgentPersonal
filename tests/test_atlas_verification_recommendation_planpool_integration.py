...
def test_plan_payload_path(tmp_path):
    ...
    r=c.post('/api/atlas/plan-pools?sync=1', json={'input':'g','project_path':str(tmp_path),'enable_repo_context':True,'plan_payload':payload})
...
def test_service_failure_non_blocking(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(mod.AtlasRepoContextAdapter, 'build_verification_recommendation', boom)
    app=create_app(); app.state.atlas_ca_data_root=str(tmp_path)
    c=TestClient(app)
    r=c.post('/api/atlas/plan-pools?sync=1', json={'input':'g','project_path':str(tmp_path),'enable_repo_context':True})
