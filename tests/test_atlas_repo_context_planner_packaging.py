...
def test_packager_missing_index_is_non_blocking(tmp_path):
    ...
    pkg = ProjectIntelligenceRepoContextPackager(data_root=tmp_path).build_package(req)
...
def test_impacted_test_recommendation_is_suggestion_only(tmp_path):
    ...
    rec = ProjectIntelligenceRepoContextPackager(data_root=tmp_path).build_impacted_test_recommendation(req)
