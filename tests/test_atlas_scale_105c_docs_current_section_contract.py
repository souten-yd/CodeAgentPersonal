...
def test_active_pr_pointer_uses_canonical_scale_106_track_tokens():
    for path in DOC_PATHS:
        ...
        if not section:
            continue
        assert 'Completed automation PR: PR-ATLAS-SCALE-111' in section
        assert 'Current automation track: PR-ATLAS-SCALE-112' in section
        assert 'Next automation track: PR-ATLAS-SCALE-112' in section
...
def test_current_vue_ui_track_state_uses_canonical_scale_106_tokens_and_forbids_stale_105_markers():
    ...
    for path in DOC_PATHS:
        ...
        if not section:
            continue
        assert 'Planned UI track: return to PR-ATLAS-SCALE-112 automation track' in section
        assert 'Current automation track: PR-ATLAS-SCALE-112' in section
        assert 'Next automation track: PR-ATLAS-SCALE-112' in section
        assert 'next work is PR-ATLAS-SCALE-112' in section
        for token in stale:
