...
def nexus_summary_payload(project: str = "default") -> dict[str, Any]:
    """Production payload for GET /nexus/summary."""
    with get_nexus_conn() as conn:
    ...
    active_jobs = sum(
        1
        for job in list_nexus_active_jobs(limit=500)
...
def nexus_documents_payload(
    project: str = "default",
    q: str = "",
    limit: int = 100,
) -> dict[str, Any]:
    """Production payload for GET /nexus/documents."""
...
def nexus_active_jobs_payload(limit: int = 50) -> dict[str, Any]:
    """Production payload for GET /nexus/jobs/active."""
    jobs = [job.model_dump(mode="json") for job in list_nexus_active_jobs(limit=limit)]
    return {"jobs": jobs}
...
def nexus_web_status_payload() -> dict[str, Any]:
    ...
    searxng_state, searxng_state_message = _resolve_searxng_state(
...
def get_system_usage_info(debug_mode: bool = False) -> dict:
    return collect_system_usage_info(
        ports=app.state.system_usage_ports,
...
def _job_execution_service_dependencies():
    return SimpleNamespace(
        resolve_effective_search_enabled=_resolve_effective_search_enabled,
        wait_threading=_wait_threading,
        job_wait_events=_job_wait_events,
        job_wait_answers=_job_wait_answers,
        job_append_step=job_append_step,
        job_update_status=job_update_status,
        job_log_append=job_log_append,
        job_log_get=job_log_get,
        resolve_runtime_llm_url=_resolve_runtime_llm_url,
        execute_chat_with_optional_web_search=execute_chat_with_optional_web_search,
        save_session=save_session,
        plan=plan,
        get_runtime_model_catalog=get_runtime_model_catalog,
        model_manager=_model_manager,
        get_model_spec=get_model_spec,
        get_coder_ladder_keys=get_coder_ladder_keys,
        run_task_mode_stream=run_task_mode_stream,
        job_option_choices=_job_option_choices,
        call_llm_chat=call_llm_chat,
        extract_json=extract_json,
        run_shell=run_shell,
        setup_venv=setup_venv,
        active_skills=_active_skills,
        upsert_skill=_upsert_skill,
        settings_get=settings_get,
        execute_task=execute_task,
        apply_ensemble_execution_mode_guard=_apply_ensemble_execution_mode_guard,
        is_quality_output_ok=_is_quality_output_ok,
        git_run=_git_run,
        ca_data_dir=CA_DATA_DIR,
        list_files=list_files,
        memory_search=memory_search,
        verify_and_fix=verify_and_fix,
        auto_snapshot_ca_data=auto_snapshot_ca_data,
        analyze_job_for_skills=analyze_job_for_skills,
        analyze_job_for_memory=_analyze_job_for_memory,
        llm_url=LLM_URL,
        choose_model_for_role=choose_model_for_role,
    )
...
def job_submit_payload(req: JobRequest):
    ...
    return submit_job_service(
        ...
        thread_factory=_job_threading.Thread,
        background_runner=run_job_background,
        current_model_key=_model_manager.current_key,
    )
...
# /skills/* routes extracted to app/api/skills.py (registered via include_routers). The
# _load_all_skills / _active_skills / _upsert_skill helpers + SKILLS_DIR constants remain here.
...
# /mcp and /mcp/info routes extracted to app/api/mcp.py (registered via include_routers).
# The TOOLS registry remains here; the router imports it lazily.
...
# /repo/* routes extracted to app/api/repo.py (registered via include_routers). The repo/cred
# helpers (repo_config_load/save, creds_load/save, _git_run, _ensure_ca_data_gitignore,
# CA_DATA_DIR) remain here; the router imports them lazily.
...
# /memory/* routes extracted to app/api/memory.py (registered via include_routers). The
# memory_search / memory_get_all / memory_save / memory_delete / _analyze_job_for_memory helpers
# remain here.
