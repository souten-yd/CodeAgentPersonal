...
class CitationSupportVerifier(Protocol):
    """Protocol for pluggable citation support verifiers."""

    def evaluate(self, *, answer_text: str, evidence_chunks: list[dict]) -> dict:
        """Return sentence-level citation support results."""


class HeuristicCitationSupportVerifier:
    """Current overlap-based citation support verifier implementation."""
    ...
    def _char_ngrams(cls, text: str, n: int = 2) -> set[str]:
        compact = re.sub(r"[\s\W_]+", "", cls._normalize_for_match(text))
        if len(compact) < n:
            return {compact} if compact else set()
        return {compact[i : i + n] for i in range(len(compact) - n + 1)}

    @classmethod
    def _sentence_evidence_score(cls, *, sentence: str, evidence_text: str) -> float:
        sentence_norm = cls._normalize_for_match(sentence)
        evidence_norm = cls._normalize_for_match(evidence_text)
        ...
        sentence_tokens = cls._tokenize(sentence_norm)
        evidence_tokens = cls._tokenize(evidence_norm)
        ...
        sentence_ngrams = cls._char_ngrams(sentence_norm)
        evidence_ngrams = cls._char_ngrams(evidence_norm)
            ...
            if not sentence_labels:
                ...
                for label in sentence_labels:
                    for chunk in chunks_by_label.get(label, []):
                        ...
                        score = self._sentence_evidence_score(sentence=sentence, evidence_text=evidence_text)
                        if score > best_score:
                            best_score = score
                ...
                if matched_evidence:
                    status = self._classify_support(best_score)
...
class LLMNLICitationSupportVerifier:
    """Future NLI-based verifier stub.

    This class intentionally preserves the same evaluate() return schema as the
    heuristic verifier for compatibility.
    """

    def evaluate(self, *, answer_text: str, evidence_chunks: list[dict]) -> dict:
        raise NotImplementedError("LLM NLI citation verifier is not implemented yet")
...
def evaluate_sentence_citations(*, answer_text: str, evidence_chunks: list[dict] | None = None) -> dict:
    ...
    verifier = HeuristicCitationSupportVerifier()
    return verifier.evaluate(answer_text=answer_text, evidence_chunks=evidence_chunks or [])


def verify_citation_labels(
    *,
    answer_text: str,
    references: list[dict],
    evidence_chunks: list[dict] | None = None,
    verifier: CitationSupportVerifier | None = None,
) -> dict:
    ...
    citation_verifier = verifier or HeuristicCitationSupportVerifier()
    semantic = citation_verifier.evaluate(answer_text=answer_text, evidence_chunks=evidence_chunks or [])
