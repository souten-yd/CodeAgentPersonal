...
def _append_operation(operations: list[dict[str, Any]], step: str, before: str, after: str) -> None:
    operations.append(
        {
            "step": step,
...
def normalize_text_for_sbv2_jp_extra(text: str | None, settings: dict | None) -> dict[str, Any]:
    ...
    # 3) URL/email policy
    before = current
    url_email_policy = str(
        settings.get("sbv2_jp_extra_url_email_policy")
        or settings.get("url_email_policy")
        or "replace"
    ).strip().lower()
    if url_email_policy == "remove":
    ...
    _append_operation(operations, "url_email_policy", before, current)

    # 4) emoji policy
    before = current
    emoji_policy = str(settings.get("sbv2_jp_extra_emoji_policy") or settings.get("emoji_policy") or "remove").strip().lower()
    ...
    current = _MULTISPACE_PATTERN.sub(" ", current).strip()
    _append_operation(operations, "emoji_policy", before, current)

    # 5) symbol policy
    before = current
    symbol_policy = str(settings.get("sbv2_jp_extra_symbol_policy") or settings.get("symbol_policy") or "replace").strip().lower()
    ...
    current = _MULTISPACE_PATTERN.sub(" ", current).strip()
    _append_operation(operations, "symbol_policy", before, current)
    ...
    before = current
    english_policy = str(settings.get("sbv2_jp_extra_english_policy") or settings.get("english_policy") or "rule").strip().lower()
    english_dict_raw = settings.get("sbv2_jp_extra_english_dict") or settings.get("english_dict") or {}
    ...
    if english_policy == "rule":
        ...
        warnings.append(f"unknown english policy: {english_policy}. fallback=rule")
        current = _ASCII_WORD_PATTERN.sub(lambda m: english_dict.get(m.group(0).lower(), m.group(0)), current)
    current = _MULTISPACE_PATTERN.sub(" ", current).strip()
    _append_operation(operations, "english_policy", before, current)
