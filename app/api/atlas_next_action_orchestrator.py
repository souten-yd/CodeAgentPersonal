...
def _svc():
    st=AtlasPlanPoolStorage('ca_data');jr=AtlasJournal('ca_data'); sis=AtlasSupervisedItemStatusService(storage=st,journal=jr); ms=AtlasMultiItemSupervisedStatusService(storage=st,journal=jr,supervised_item_status_service=sis)
    return AtlasNextActionOrchestratorService(storage=st,journal=jr,supervised_status_service=ms)
@router.get('/policies')
def policies(): return {"policies":[p.model_dump() for p in list_next_action_orchestrator_policies()]}
@router.post('/prepare')
def prepare(payload: AtlasNextActionOrchestratorRequest):
    ...
    try:
        return _svc().prepare(payload).model_dump()
...
def result(pool_id:str, orchestrator_run_id:str):
    ...
    path=Path('ca_data')/'atlas'/'next_action_orchestrator'/pid/f"{_v(orchestrator_run_id,'orchestrator_run_id')}.json"
...
def latest(payload: LatestReq):
    pid=_v(payload.pool_id,'pool_id'); root=Path('ca_data')/'atlas'/'next_action_orchestrator'/pid
