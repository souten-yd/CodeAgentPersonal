...
@router.post('/impacts')
def impacts(payload:AtlasRepoIndexRequest, request:Request): _validate_project_path(payload.project_path); return _svc(request).query_impacts(payload.project_path,payload.changed_files)
@router.post('/related-tests')
def related(payload:AtlasRepoIndexRequest, request:Request): _validate_project_path(payload.project_path); return _svc(request).query_related_tests(payload.project_path,payload.changed_files)
@router.post('/latest')
def latest(payload:AtlasRepoIndexRequest, request:Request): _validate_project_path(payload.project_path); return _svc(request).load_latest(payload.workspace_id,payload.project_path)
...
def result(project_hash:str,index_run_id:str,request:Request):
    ...
    data=AtlasRepoIndexStorage(resolve_atlas_ca_data_root(request)).load_result_by_hash(project_hash,index_run_id)
