...
def _svc(request: Request):
    ...
    st=AtlasPlanPoolStorage(root); jr=AtlasJournal(root); sis=AtlasSupervisedItemStatusService(storage=st,journal=jr)
    return AtlasMultiItemSupervisedStatusService(storage=st,journal=jr,supervised_item_status_service=sis,data_root=root)
@router.post('/build')
def build(payload: AtlasMultiItemSupervisedStatusRequest, request: Request):
    ...
    return _svc(request).build_status(payload).model_dump()
@router.get('/results/{pool_id}/{multi_status_run_id}')
def result(pool_id: str, multi_status_run_id: str, request: Request):
    ...
    root = resolve_atlas_ca_data_root(request)
    path=root/'atlas'/'multi_item_supervised_status'/pid/f"{_v(multi_status_run_id,'multi_status_run_id')}.json"
...
def latest(payload: LatestReq, request: Request):
    pid=_v(payload.pool_id,'pool_id'); root=resolve_atlas_ca_data_root(request)/'atlas'/'multi_item_supervised_status'/pid
