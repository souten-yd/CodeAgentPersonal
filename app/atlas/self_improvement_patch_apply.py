...
def apply_self_improvement_patch_one_action(
    *,
    ...
    if blocked or dry_run:
        return validate_self_improvement_patch_apply(base)
    ...
    result = {**base, "actual_file_changed": actual_changed, "mutation_performed": actual_changed, "applied_at": _utc_now()}
...
def validate_self_improvement_patch_apply(result: dict[str, Any]) -> dict[str, Any]:
    ...
    invariants = {
        ...
        "status": status in {"blocked", "planned", "applied"},
        ...
        "verification_result_fabricated": result.get("verification_result_fabricated") is False,
        ...
        "blocked_reasons": (not blocked) or bool(result.get("blocked_reasons")),
        "changed_files": (not applied) or len(result.get("changed_files", [])) == 1,
    }
