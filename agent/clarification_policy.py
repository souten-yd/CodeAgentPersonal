...
class ClarificationPolicy:
    def classify(self, *, user_input: str, task_type: str, requirement_mode: str, project_context: str = "") -> ClarificationDecision:
        ...
        research_terms = ["最新", "ニュース", "論文", "仕様", "価格", "web", "website", "api version", "benchmark"]
        if any(t in text for t in research_terms):
            return ClarificationDecision("not_needed", "research_needed")
        ...
        if any(k.lower() in text for k in required_keywords) or "ios" in context or "iphone safari" in context:
            return ClarificationDecision("required", "user_decision_needed")
