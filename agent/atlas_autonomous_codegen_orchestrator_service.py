...
class AtlasAutonomousCodegenOrchestratorService:
    ...
    def _run_interleaved_items(
        ...
        *,
        ...
        # Items that failed generation with MISSING CONTENT (capability), eligible for create-mode
        # section recovery. Distinct from hard-blocked (policy: critical/delete/forbidden) items,
        # which must NEVER be re-generated by recovery.
        recoverable_no_content_ids: list[str] = []
        ...
        for index, item_id in enumerate(selected_ids):
            ...
            if request.generate_missing_patches and (stale or not self._item_has_patch_content(item)):
                ...
                if available:
                    ...
                    out.skipped_generation_count += 1
                    no_content_item_ids.append(item_id)
                    recoverable_no_content_ids.append(item_id)
        ...
        # ── Create-mode section recovery (deferred to the end of this wave, per design): each item
        # this wave could not generate whole-file (no_content) is rebuilt via the skeleton +
        # per-section builder — small calls a weak model can do where the one-shot whole-file failed —
        # then the recovered ones are applied together. Bounded (the builder caps sections/retries and
        # ends in capability_ceiling, never a loop), gated by ATLAS_CREATE_MODE_SECTION_RECOVERY
        # (default on; only runs after a real failure so there is no normal-path cost), disabled-safe.
        import os
        if recoverable_no_content_ids and os.environ.get("ATLAS_CREATE_MODE_SECTION_RECOVERY", "1").strip() != "0":
            recovered_ready: list[str] = []
            for rid in list(recoverable_no_content_ids):
                rpool = self.storage.load_pool(request.pool_id)
                ritem = rpool.get_item(rid)
                if ritem is None or self._item_has_patch_content(ritem):
                    continue
                self._emit("autonomous_codegen_section_recovery_started", request.pool_id, run_id, orchestrator_run_id, item_id=rid)
                rec = self.patch_proposal_service.propose_for_item(
                    AtlasPatchProposalRequest(
                        pool_id=request.pool_id, item_id=rid, run_id=run_id,
                        workspace_id=request.workspace_id, source_type="plan_item",
                        metadata={**self._twin_generation_hints(out), "create_mode_section_recovery": True},
                    )
                )
                if is_patch_generation_success((rec.metadata or {}).get("patch_generation")):
                    recovered_ready.append(rid)
                    out.generated_count += 1
                    out.proposal_results.append(AtlasAutonomousCodegenProposalResult(item_id=rid, status="proposed", patch_content_available=True, reason="create_mode_section_recovery"))
                else:
                    out.proposal_results.append(AtlasAutonomousCodegenProposalResult(item_id=rid, status="no_content", patch_content_available=False, reason=(rec.warnings[0] if rec.warnings else "capability_ceiling")))
            if recovered_ready:
                rec_run = self.multi_item_autopilot_service.run(
                    AtlasMultiItemAutopilotRequest(
                        pool_id=request.pool_id, run_id=run_id, workspace_id=request.workspace_id,
                        project_path=project_path, item_ids=recovered_ready, policy_id=request.policy_id,
                        require_approval=False, stop_on_manual_required=stop_on_manual_required,
                        max_items=len(recovered_ready), max_runtime_seconds=effective_max_runtime_seconds,
                        max_changed_files_total=max(0, effective_max_changed_files_total - changed_total),
                        include_context_refresh=True, include_evaluator=True, include_bounded_retry=True,
                        include_self_correction=True, include_correction_routing=True, include_harness_provisioning=True,
                        metadata={**(request.metadata or {}), "data_root": str(self.data_root), "orchestrator_run_id": orchestrator_run_id, "interleaved_parent_autopilot_run_id": aggregate.autopilot_run_id},
                    )
                )
                self._merge_item_autopilot_result(aggregate, rec_run)
                self._persist_completed_item_evidence(request.pool_id, rec_run)
                changed_total = len(self._changed_files_from_autopilot(aggregate))
                applied_ok = {r.item_id for r in rec_run.item_results if r.status in {"completed", "applied_no_verification"}}
                no_content_item_ids = [x for x in no_content_item_ids if x not in applied_ok]
                if applied_ok:
                    aggregate.metadata["create_mode_section_recovered"] = sorted(applied_ok)
                    if aggregate.status == "no_content":
                        aggregate.status = "completed" if not no_content_item_ids else "partial"
