...
class AtlasPostManualExecutionRefreshService:
    def __init__(self, *, storage, journal, supervised_item_status_service, multi_status_service, next_action_orchestrator_service, data_root: str | Path | None = None):
        self.storage=storage; self.journal=journal; self.supervised_item_status_service=supervised_item_status_service; self.multi_status_service=multi_status_service; self.next_action_orchestrator_service=next_action_orchestrator_service; self.data_root=Path(data_root or "ca_data").expanduser().resolve()
    ...
    def _load_manual(self,pool_id,executor_run_id):
        p=self.data_root/"atlas"/"manual_next_action_executor"/validate_relative_path(pool_id)/f"{validate_relative_path(executor_run_id)}.json"
    ...
    def _save(self, req, r):
        ...
        json_rel=f"atlas/post_manual_execution_refresh/{r.pool_id}/{r.refresh_run_id}.json"
        root=(self.data_root/"atlas"/"post_manual_execution_refresh"/req.pool_id); root.mkdir(parents=True, exist_ok=True)
        r.metadata.update({"result_path": str(self.data_root/json_rel), "result_path_relative": json_rel})
