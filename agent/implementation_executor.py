...
class ImplementationExecutor:
    ...
    def _run_step(self, run: ImplementationRun, step: ImplementationStepResult, execution_mode: str, plan: dict, project_path: Path | None, allow_update: bool, allow_create: bool, logs: list[str], apply_patches: bool, preview_only: bool, max_patch_bytes: int, patch_generation_mode: str = "llm_replace_block") -> None:
        ...
        try:
            ...
            if action == "create":
                ...
                self._block(step, run, "create requires full-content generation; skeleton creation is disabled"); return
            if action == "update":
    ...
    def _create_stub(self, step: ImplementationStepResult, project_path: Path) -> None:
        raise RuntimeError("skeleton create path is disabled; use full-content generation")

    def _safe_update(self, step: ImplementationStepResult, project_path: Path, run_id: str, plan_id: str, plan: dict, apply_patches: bool, preview_only: bool, max_patch_bytes: int, patch_generation_mode: str = "llm_replace_block") -> None:
        ...
        proposal = None
        llm_telemetry_id = ""
        mode = (patch_generation_mode or "llm_replace_block").strip().lower()
        try:
            if mode in {"llm_replace_block", "auto"}:
                ...
                if mode == "auto" and not proposal.apply_allowed:
                    ...
                    proposal.metadata = {**(proposal.metadata or {}), "fallback_from": "llm_replace_block", "fallback_reason": fallback_reason, "fallback_telemetry_id": llm_telemetry_id, "append_fallback_disabled": True}
            else:
                self._block(step, None, f"patch_generation_mode={mode} is disabled; append fallback is not allowed")
                return
        ...
        if not proposal.apply_allowed:
            self._block(step, None, f"patch generation blocked: {proposal.can_apply_reason or 'proposal_not_applicable'}")
            return
        if preview_only:
    ...
    def apply_patch(self, run_id: str, patch_id: str) -> dict:
        ...
        if patch_type == "append":
            raise ValueError("append patch apply is disabled")
        ...
        ar = PatchApplyResult(
            ...
            changed_bytes=len(str(patch.get("replacement_block", "")).encode("utf-8")),
