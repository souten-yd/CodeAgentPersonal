...
STYLE_BERT_VENV_DIR="${CODEAGENT_STYLE_BERT_VITS2_VENV_DIR:-${STYLE_BERT_REPO_DIR}/.venv}"
...
if [[ ! -d "${STYLE_BERT_VENV_DIR}" && -d "${STYLE_BERT_VENV_FALLBACK}" ]]; then
  mkdir -p "$(dirname "${STYLE_BERT_VENV_DIR}")"
  echo "[Runpod][SBV2] Workspace venv missing. Copying ${STYLE_BERT_VENV_FALLBACK} -> ${STYLE_BERT_VENV_DIR}"
  cp -a "${STYLE_BERT_VENV_FALLBACK}" "${STYLE_BERT_VENV_DIR}"
fi

if [[ ! -d "${STYLE_BERT_VENV_DIR}" ]]; then
...
echo "[Runpod][SBV2] venv_dir=${CODEAGENT_STYLE_BERT_VITS2_VENV_DIR} (fallback=${STYLE_BERT_VENV_FALLBACK})"
