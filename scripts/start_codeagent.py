...
def ensure_llama_server(base_dir: Path, runpod: bool) -> None:
    ...
    if backend not in ("cuda", "vulkan", "hip"):
        print("[Runpod][WARN] Unsupported/unknown GPU backend. Skip llama release auto-install.")
        return
    ...
    try:
        ...
        if backend == "cuda":
            ...
            if setup_script.exists():
                print(f"[Runpod] Fallback to CUDA setup script: {setup_script}")
                try:
                    subprocess.run(
                    ...
                    print(f"[Runpod][WARN] llama setup script failed: {e2}")
                    return
            else:
