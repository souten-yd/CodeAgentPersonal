...
### Qwen3-TTS 依存（torch/torchaudio）は別ファイルで導入

`requirements.txt` には `torch` / `torchaudio` を直接固定していません。  
Qwen3-TTS を使う場合は `requirements-tts.txt` を使って、実行環境に合わせて導入してください。

```bash
# CPU環境（Docker / ローカル共通）
python -m pip install -r requirements-tts.txt --index-url https://download.pytorch.org/whl/cpu

# GPU環境（CUDA 12.4, Docker / ローカル共通）
python -m pip install -r requirements-tts.txt --index-url https://download.pytorch.org/whl/cu124
```

補足:
- Dockerビルド時は `requirements-tts.txt` の導入を試行し、結果を `/app/qwen3_tts_install_status.json` に保存します。
- `GET /tts/status` の `qwen3tts_install_status` / `qwen3tts_missing_requirements` で不足理由を確認できます。
- `--build-arg QWEN3_TTS_REQUIRED=true` を指定すると、Qwen3-TTS依存導入に失敗した時点で Docker build を失敗させられます。
...
| Qwen3-TTS | ローカル高品質多言語 TTS + ボイスクローン対応 | `requirements-tts.txt` を CPU/GPU 別に導入（上記手順） |
