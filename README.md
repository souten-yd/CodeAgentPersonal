...
- Phase 29.0b extends approve-button detection diagnostics with button inventory + broader selectors, still detection-only (no approve/execute/apply clicks).
...
- Real backend E2E command (backend already running): `PLAYWRIGHT_SMOKE_BASE_URL=http://127.0.0.1:8000 RUN_ATLAS_BACKEND_E2E=1 python scripts/smoke_ui_modes_playwright.py`
- Artifact summary is available in GitHub Actions (`artifacts/playwright/summary.md` + step summary).

## Playwright smoke (backend preflight / full E2E opt-in)

- Default `python scripts/smoke_ui_modes_playwright.py` runs mock-backed 9 UI scenarios only (no backend preflight/E2E).
- Backend preflight only (GET-only):
  - `RUN_ATLAS_BACKEND_PREFLIGHT=1` runs only `atlas_backend_preflight` (skips default 9 UI scenarios).
  - Does not press Atlas Start and does not start planner/LLM.

```bash
PLAYWRIGHT_SMOKE_BASE_URL=http://127.0.0.1:8000 RUN_ATLAS_BACKEND_PREFLIGHT=1 python scripts/smoke_ui_modes_playwright.py
```
...
- Full backend E2E (preflight runs first):
  ...
  - Windows local full backend E2E dry-run has been validated.
  - Expected opt-in result is 2/2 PASS (`atlas_backend_preflight`, `atlas_backend_e2e_journey`).
  - This mode remains manual opt-in only.
  - This dry-run does not approve / execute / apply patches.
...
- Phase 28.0: clarification resolution is explicit opt-in only (`RUN_ATLAS_BACKEND_E2E=1` + `RUN_ATLAS_BACKEND_E2E_WAIT_PLAN=1` + `RUN_ATLAS_BACKEND_E2E_RESOLVE_CLARIFICATION=1`), clicks `おまかせで進める` at most once, never auto-answers clarification, and stops at `completed` or `needs_clarification_after_resolution` without approval/execute/apply.
