...
- `RUNPOD_AUTO_SETUP_LLAMA=true` (既定): `llama-server` 不在時に `scripts/setup_llama_runpod.sh` で llama.cpp を **CUDAソースビルド** して `/workspace/llama` へ配置
...
- `LLAMA_CUDA_FLAGS` (既定: `-Wno-deprecated-gpu-targets`): nvcc の deprecated-gpu-target 警告を抑制したい場合の追加フラグ指定
- `LLAMA_CUDA_PREBUILT_URL`: 指定時は CUDA プリビルド tar.gz のDL/展開を優先し、未指定時のみソースビルド（ビルド時間短縮向け）
...
- ランチャー (`scripts/start_codeagent.py`) は Runpod で `llama-server` が見つからない場合、`scripts/setup_llama_runpod.sh` を実行して **CUDAビルド** を作成し、`LLAMA_SERVER_PATH` を自動設定します。
- `scripts/setup_llama_runpod.sh` は **手動運用向け** の CUDA 導入スクリプトです（CIや特殊環境向け）。
- CUDA専用の軽量 multi-stage イメージ例は `Dockerfile.llama-cuda` を参照（devel でビルド→runtime へ `llama-server`/`llama-cli` のみコピー）。
  - `--build-arg LLAMA_PREBUILT_URL=<公式CUDAプリビルドtar.gz>` を指定するとソースコンパイルをスキップして高速化できます。
...
```bash
# 通常は不要（ランチャーが CUDA ソースビルドを自動導入）
python scripts/start_codeagent.py

# 明示的に再ビルドする場合のみ:
# bash scripts/setup_llama_runpod.sh --force-build
```
