...
- `scripts/setup_llama_runpod.sh` は最新のCUDA版プリビルドを取得後、`llama-server --version` と `ldd` で CUDA ランタイムリンクを検証します。検証NG時は `--build-if-needed` 指定で GitHub Actions 内ビルド（`GGML_CUDA=ON`）へ自動フォールバックします。
- 同スクリプトは Python 3.11 向け `llama-cpp-python` CUDA wheel も検証し、不在/非CUDA版の場合は CUDA wheel を自動インストールします（`--skip-python-wheel` で無効化可能）。
...
3.5. llama.cpp CUDA の導入（プリビルド検証 + 必要時ビルド）

```bash
bash scripts/setup_llama_runpod.sh --build-if-needed
# 必要なら Python 3.11 向け llama-cpp-python CUDA wheel も同時に導入されます
```
