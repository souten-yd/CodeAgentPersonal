...
- ランチャー (`scripts/start_codeagent.py`) は Runpod で `llama-server` が見つからない場合、VulkanプリビルドをDLして `/workspace/llama` へ展開し、`LLAMA_SERVER_PATH` を自動設定します。
...
```bash
# 通常は不要（ランチャーが Vulkan プリビルドを自動導入）
python scripts/start_codeagent.py
