...
${preview}`;
  ...
  function renderOperatorLoopVerificationHandoff(){ const h=getOperatorLoopVerificationHandoff(); const s=$('atlas-operator-loop-verification-handoff-summary'); const r=$('atlas-operator-loop-verification-handoff-result'); const impacted=Array.isArray(h.impacted_files)?h.impacted_files:[]; const tests=Array.isArray(h.related_tests)?h.related_tests:[]; const cmds=Array.isArray(h.recommended_commands)?h.recommended_commands:[]; const steps=Array.isArray(h.manual_verification_steps)?h.manual_verification_steps:[]; const warns=Array.isArray(h.warnings)?h.warnings:[]; const summary={approval_summary:h.approval_summary||'',confidence:h.confidence||'unknown',impacted_files_count:impacted.length,impacted_files:impacted,related_tests_count:tests.length,related_tests:tests,recommended_commands_count:cmds.length,recommended_commands:cmds,manual_verification_steps_count:steps.length,manual_verification_steps:steps,warnings_count:warns.length,warnings:warns,advisory_only:h.advisory_only===true,manual_approval_only:h.manual_approval_only===true,executed:h.executed===true}; if(s) s.textContent=`Verification Handoff Summary (manual approval context): approval_summary=${summary.approval_summary||'-'} / confidence=${summary.confidence} / impacted_files=${summary.impacted_files_count} / related_tests=${summary.related_tests_count} / recommended_commands=${summary.recommended_commands_count} / manual_verification_steps=${summary.manual_verification_steps_count} / warnings=${summary.warnings_count} / advisory_only=${String(summary.advisory_only)} / manual_approval_only=${String(summary.manual_approval_only)} / executed=${String(summary.executed)}`; if(r) r.textContent=JSON.stringify(summary,null,2); }
  function operatorLoopRender(){ const st=$('atlas-operator-loop-status'), why=$('atlas-operator-loop-disabled-reason'), step=$('atlas-operator-loop-current-step'), sum=$('atlas-operator-loop-next-action-summary'), exBtn=$('atlas-operator-loop-explicit-decision'), exReq=operatorLoopState.selectedNextAction==='approve_patch_candidate', pv=operatorLoopState.lastContractResult?.action_contract?.payload_valid===true, dry=operatorLoopState.lastDryRunResult||{}, dryReady=!!(operatorLoopState.dryRunExecutorRunId&&dry.status==='dry_run'&&dry.validation&&dry.validation.executable===true), confirmOk=operatorLoopState.confirmationText==='EXECUTE ONE ACTION'&&!!operatorLoopState.confirmationToken, execDecisionOk=!exReq||operatorLoopState.explicitDecision==='approve', reasons=[], g={build:!operatorLoopState.poolId,prepare:!(operatorLoopState.poolId&&operatorLoopState.multiStatusRunId),token:!(operatorLoopState.orchestratorRunId&&operatorLoopState.actionId&&operatorLoopState.selectedNextAction&&operatorLoopState.selectedItemId&&operatorLoopState.actionKind==='execution_candidate'),dry:!(confirmOk&&operatorLoopState.actionKind==='execution_candidate'&&pv&&execDecisionOk),execute:!(dryReady&&confirmOk&&execDecisionOk),refresh:!(operatorLoopState.executedExecutorRunId||operatorLoopState.dryRunExecutorRunId)}; [['build-queue',g.build],['prepare',g.prepare],['token',g.token],['dry-run',g.dry],['execute',g.execute],['refresh',g.refresh],['advance',!operatorLoopState.poolId],['execute-refresh',g.execute||operatorLoopState.actionKind!=='execution_candidate']].forEach(([k,v])=>{ const b=$('atlas-operator-loop-'+k+'-btn'); if(b) b.disabled=!!v; }); if(exBtn){ exBtn.disabled=!exReq; if(!exReq) exBtn.value=''; } if(g.build) reasons.push('Build Queue requires pool_id.'); if(g.prepare) reasons.push('Prepare requires multi_status_run_id.'); if(g.token) reasons.push('Preview Token requires action_ready contract.'); if(g.dry) reasons.push('Dry Run requires confirmation token and payload_valid=true.'); if(g.execute) reasons.push('Execute requires successful dry_run.'); if(g.refresh) reasons.push('Refresh requires executor_run_id.'); if(!operatorLoopState.poolId) reasons.push('Advance requires pool_id.'); if(g.execute||operatorLoopState.actionKind!=='execution_candidate') reasons.push('Execute and refresh requires successful dry_run and confirmation.'); if(exReq&&operatorLoopState.explicitDecision!=='approve') reasons.push('Approval action requires explicit_decision=approve.'); if(st) st.textContent='Operator loop ready'; if(why) why.textContent=reasons.join(' | ')||'All step guards satisfied.'; if(step) step.textContent=`Current step: ${dryReady?'ready_to_execute':(operatorLoopState.actionId?'action_prepared':'idle')}.`; if(sum) sum.textContent=`Next action: item=${operatorLoopState.selectedItemId||'-'} / action=${operatorLoopState.selectedNextAction||'-'} / kind=${operatorLoopState.actionKind||'-'}.`; const d=$('atlas-operator-loop-diagnostics'); if(d) d.textContent=JSON.stringify({selectedItemId:operatorLoopState.selectedItemId,selectedNextAction:operatorLoopState.selectedNextAction,actionId:operatorLoopState.actionId,actionKind:operatorLoopState.actionKind,payloadValid:pv,requiresExplicitDecision:exReq,explicitDecision:operatorLoopState.explicitDecision||'',guards:g,reasons:reasons},null,2); renderOperatorLoopVerificationHandoff(); }
