...
function resolveNexusResearchAutoSettings({ searchType, depth } = {}) {
  ...
  const byDepth = {
    ...
    standard: { max_queries: 5, max_results_per_query: 8, max_sources: 80, max_downloads: 40, recursive_search: false, max_iterations: 1, max_followup_queries: 4, confidence_threshold: 0.76, continue_on_download_error: true, stop_when_sufficient: true },
    deep: { max_queries: 8, max_results_per_query: 12, max_sources: 180, max_downloads: 120, max_total_download_mb: 2048, max_download_mb: 80, download_timeout_sec: 45, target_candidate_count: 300, target_valid_source_count: 60, target_evidence_count: long64k ? 220 : 180, target_high_quality_source_count: 16, target_official_source_count: 8, target_pdf_source_count: 8, max_retrieval_rounds: 4, adaptive_retrieval_enabled: true, recursive_search: true, max_iterations: 2, max_followup_queries: 8, confidence_threshold: long64k ? 0.82 : 0.78, continue_on_download_error: true, stop_when_sufficient: true },
    exhaustive: { max_queries: 12, max_results_per_query: 15, max_sources: 300, max_downloads: 200, max_total_download_mb: 4096, max_download_mb: 100, download_timeout_sec: 60, target_candidate_count: 500, target_valid_source_count: 100, target_evidence_count: long64k ? 340 : 300, target_high_quality_source_count: 24, target_official_source_count: 14, target_pdf_source_count: 14, max_retrieval_rounds: 6, adaptive_retrieval_enabled: true, recursive_search: true, max_iterations: 4, max_followup_queries: 12, confidence_threshold: 0.85, continue_on_download_error: true, stop_when_sufficient: true },
  };
  const typeMap = {
...
function formatNexusResearchStatusCompact(job = {}, bundle = {}, answer = {}) {
  ...
  const answerGenerated = Boolean(answer?.answer_generated || String(answer?.answer_markdown || answer?.answer || '').trim());
